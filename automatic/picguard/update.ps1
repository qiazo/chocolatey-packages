...
function global:au_GetLatest {
    ...
    $LatestURL64 = ($LatestRelease.assets | Where-Object {$_.name.StartsWith("picguard-") -and -not $_.name.StartsWith("picguard-pro-") -and $_.name.EndsWith("-windows-setup-x64.exe")}).browser_download_url
    $LatestURLArm64 = ($LatestRelease.assets | Where-Object {$_.name.StartsWith("picguard-") -and -not $_.name.StartsWith("picguard-pro-") -and $_.name.EndsWith("-windows-setup-arm64.exe")}).browser_download_url

    if (!$LatestURL64) {
        throw "X64 URL is missing!"
    }
    ...
    $LatestChecksum64    = Get-RemoteFileSha256 -FileUrl $LatestURL64 -TempPath "$env:TEMP\picguard-$($LatestVersion)-windows-setup-x64.exe"
    $LatestChecksumArm64 = Get-RemoteFileSha256 -FileUrl $LatestURLArm64 -TempPath "$env:TEMP\picguard-$($LatestVersion)-windows-setup-arm64.exe"
