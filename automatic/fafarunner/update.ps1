...
# add headers
$headers = @{
    "Authorization" = "Bearer $Env:FR_RELEASE_TOKEN"
...
function global:au_GetLatest {
    $LatestRelease = Invoke-RestMethod -UseBasicParsing -Uri "https://api.github.com/repos/fafarunner/fafarunner/releases/latest" -Headers $headers
    ...
    $LatestChecksum64    = Get-RemoteFileSha256 -FileUrl $LatestURL64 -TempPath "$env:TEMP\fafarunner-$($LatestVersion)-windows-setup-x64.exe"
    $LatestChecksumArm64 = Get-RemoteFileSha256 -FileUrl $LatestURLArm64 -TempPath "$env:TEMP\fafarunner-$($LatestVersion)-windows-setup-arm64.exe"
