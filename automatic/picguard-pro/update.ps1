...
function global:au_GetLatest {
    ...
    $LatestURLArm64 = ($LatestRelease.assets | Where-Object {$_.name.StartsWith("picguard-pro-") -and $_.name.EndsWith("-windows-setup-arm64.exe")}).browser_download_url
    ...
    $LatestChecksum64    = Get-RemoteFileSha256 -FileUrl $LatestURL64 -TempPath "$env:TEMP\picguard-pro-$($LatestVersion)-windows-setup-x64.exe"
    $LatestChecksumArm64 = Get-RemoteFileSha256 -FileUrl $LatestURLArm64 -TempPath "$env:TEMP\picguard-pro-$($LatestVersion)-windows-setup-arm64.exe"
